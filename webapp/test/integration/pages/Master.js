				...
				iRememberAnIdOfAnObjectThatsNotInTheList: function () {
					return this.waitFor({
						...
						success: function (oList) {
							var aEntityData = this.getEntitySet("BusinessPartnerCollection");
							...
							if (aObjectsNotInTheList.length) {
								sObjectNotInTheListId = aObjectsNotInTheList[0].BusinessPartnerKey;
							} else {
								// Not enough items all of them are displayed so we take the last one
								sObjectNotInTheListId = aEntityData[aEntityData.length - 1].BusinessPartnerKey;
							}
							...
							oCurrentItem.bindingPath = "/" + oList.getModel().createKey("BusinessPartnerCollection", {
								BusinessPartnerKey: sObjectNotInTheListId
							});
			...
			assertions: {
				...
				theListHeaderDisplaysZeroHits: function () {
					return this.waitFor({
						...
						matchers: new PropertyStrictEquals({
							name: "text",
							value: "<BusinessPartnerCollection> (0)"
						}),
						success: function () {
				...
				theListShouldHaveAllEntries: function () {
					return this.waitFor({
						...
						viewName: sViewName,
						success: function (oList) {
							var aAllEntities = this.getEntitySet("BusinessPartnerCollection");
					...
					return this.waitFor({
						...
						success: function (oList) {
							...
							return this.waitFor({
								...
								matchers: new PropertyStrictEquals({
									name: "text",
									value: "<BusinessPartnerCollection> (" + iExpectedLength + ")"
								}),
								success: function () {
